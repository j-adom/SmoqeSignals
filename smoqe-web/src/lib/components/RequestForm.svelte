	...
	const copy = $derived(
		{
			...
			catering: {
				...
				sending: 'Sending…',
				confirmed:
					"we've got your details. We'll confirm availability and dial in the menu shortly."
			}
		}[context]
	);
...
<div class="card-brand p-[30px]">
	{#if form?.success}
		<div class="bg-flame/15 text-flame mb-[18px] grid h-14 w-14 place-items-center rounded-full">
			<Check size={30} />
		</div>
		<h3 class="display text-3xl">Request received</h3>
		<p class="text-ink-soft mt-3 mb-[18px] leading-relaxed">
			Thanks{form.name ? `, ${String(form.name).split(' ')[0]}` : ''}{form.eventDate
				? ` — for ${formatDate(String(form.eventDate))},`
				: ' —'}
			{copy.confirmed}
		</p>
		<div class="bg-paper-2 text-ink-soft rounded px-[18px] py-4 text-sm leading-7">
		...
		<div class="mt-[22px] flex flex-wrap gap-3">
			...
			<a href={newRequestHref} class="btn btn-ghost border-paper-line flex-1">New Request</a>
		</div>
	{:else}
		<h3 class="display text-[28px]">{copy.heading}</h3>
		<p class="text-ink-soft mt-1 mb-5 text-[14.5px]">{copy.sub}</p>
